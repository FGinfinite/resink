...
- `start_agent_task` -- start a bounded `citation-assistant` task for batch citation audits
...
## Batch Operations via start_agent_task

For the following tasks, use `start_agent_task` with `capabilityName: "citation-assistant"`:
...
- `objective`: the concrete citation audit or bibliography operation
- `acceptanceCriteria`: include exact files checked and a structured issue list
- `input.files`: target `.bib` and related `.tex` files
- `policy.tools`: `["read_document", "list_files", "search_project", "bib_lookup", "bib_manage"]`
