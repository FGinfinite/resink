---
name: review
description: Deep paper review (deterministic workflow graph with parallel reviewers)
triggerHint: When the user asks to review, critique, or evaluate a paper
---

# 深度论文审阅技能 (Deep Review)

你现在进入**深度论文审阅工作流**。你应启动内置 Deep Review agent team workflow，而不是手动顺序调度子 agent。

## 可用工具

- `list_files` -- 列出项目中的所有文件，了解项目结构
- `read_document` -- 读取指定文档（使用 path 参数读取 .tex 文件）
- `start_agent_team` -- 启动确定性的 Deep Review workflow graph
...
```json
{
  ...
  "userRequest": "结合用户要求和你的论文初步分析，对整篇论文做 Deep Review。",
  "targetVenue": "用户指定的目标会议/期刊，如未指定可省略",
  "files": [
    {
      "path": "main.tex",
      "mode": "excerpt",
      "reason": "main paper entry point"
    }
  ]
}
```

不要手动顺序委派评审任务。Deep Review workflow 会并行运行 `content-reviewer`、`experiment-reviewer`、`quality-checker`，然后运行 reducer 和 critic。

### Phase 3: 综合报告

收到 `start_agent_team` 结果后，基于 reducer/critic 输出向用户给出最终结构化报告。在综合时：
- 使用四维评估框架逐维度给出简要评价
