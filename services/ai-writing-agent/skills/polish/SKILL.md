...
- `references/style-guide.md`: concise polishing checklist and smoke-test marker guidance.

Use `run_skill_script` only when a lightweight pre-edit scan would materially help:
- `latex_sanity_report.py`: read-only scan for word count, long-sentence hints, and passive-voice hints in a workspace `.tex` file.
