...
- `start_agent_task` -- 启动有边界的 `document-auditor` 结构化审计任务
...
使用 `start_agent_task` 启动 `document-auditor`，结构化任务要求：
- 执行完整的交叉引用审计（labels、refs、cites）
- 重点检查未定义的引用和缺失的 citation key
- `objective`: 完整审计交叉引用、citation key 和文档结构完整性
- `acceptanceCriteria`: 返回按严重程度分组的问题列表，每个问题必须包含文件/位置和修复建议
- `policy.tools`: `["read_document", "list_files", "search_project", "label_ref_audit", "doc_structure_map"]`
