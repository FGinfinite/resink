...
const CompletionRulesEditor = memo(function CompletionRulesEditor({
  isOpen,
  onClose,
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaveStatus('idle')
    try {
      await aiApi.updateCompletionRules(projectId, content)
      setSaveStatus('saved')
      if (saveTimerRef.current) clearTimeout(saveTimerRef.current)
      ...
      <div className="ai-project-rules-body">
        {loading ? (
          ...
          <textarea
            ...
            maxLength={maxLength}
            placeholder={'在此编写补全偏好，自动补全和强大补全都会遵循这些规则...\n\n示例：\n- 只补全一句话\n- 用学术化的语言\n- 使用英文补全'}
          />
        ) : (
...
export default CompletionRulesEditor
