...
Object.values(Settings.i18n.subdomainLang || {}).forEach(function (spec) {
  ...
  //  middleware — only keep the first mapping per host to avoid
  //  cookie-only languages (sharing the same siteUrl) from overriding
  //  the default host language
  const host = new URL(spec.url).host
  if (!availableHosts.has(host)) {
    availableHosts.set(host, spec.lngCode)
  }
...
// Cookie-based language detector (highest priority)
const cookieLangDetector = new middleware.LanguageDetector(i18n.services, {
  order: ['cookie'],
  lookupCookie: 'i18next',
})

// Accept-Language header detector (for suggestion banner)
...
function setLangBasedOnDomainMiddleware(req, res, next) {
  // Priority: cookie > host
  // Validate cookie value against supported languages to prevent stale/invalid
  // cookies from suppressing the suggestion banner or setting unsupported langs
  const rawCookieLang = cookieLangDetector.detect(req, res)
  const cookieLang =
    rawCookieLang && availableLanguageCodes.includes(rawCookieLang)
      ? rawCookieLang
      : null
  const hostLang = availableHosts.get(req.headers.host)
  const lang = cookieLang || hostLang
  ...
  // Only show the language suggestion banner if the user hasn't already
  // made a choice via cookie
  if (!cookieLang) {
