...
const rateLimiters = {
  ...
  internalEntityOps: new RateLimiter('internal-entity-ops', {
    points: 120,
    duration: 60,
  }),
}
...
export default {
  apply(webRouter, privateApiRouter) {
    ...
    // Internal API for AI agent - entity operations
    privateApiRouter.post(
      '/internal/project/:Project_id/doc',
      AuthenticationController.requirePrivateApiAuth(),
      RateLimiterMiddleware.rateLimit(rateLimiters.internalEntityOps, {
        params: ['Project_id'],
      }),
      EditorHttpController.internalAddDoc
    )

    privateApiRouter.delete(
      '/internal/project/:Project_id/:entity_type/:entity_id',
      AuthenticationController.requirePrivateApiAuth(),
      RateLimiterMiddleware.rateLimit(rateLimiters.internalEntityOps, {
        params: ['Project_id'],
      }),
      EditorHttpController.internalDeleteEntity
    )

    privateApiRouter.post(
      '/internal/project/:Project_id/folder',
      AuthenticationController.requirePrivateApiAuth(),
      RateLimiterMiddleware.rateLimit(rateLimiters.internalEntityOps, {
        params: ['Project_id'],
      }),
      EditorHttpController.internalAddFolder
    )
  },
}
