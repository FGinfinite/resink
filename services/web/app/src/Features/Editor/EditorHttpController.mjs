...
export default {
  ...
  internalAddDoc: expressify(internalAddDoc),
  internalDeleteEntity: expressify(internalDeleteEntity),
  internalAddFolder: expressify(internalAddFolder),
...
/**
 * Validate that a string is a valid 24-character hex ObjectId.
 */
...
/**
 * Validate req.body.userId for internal API endpoints.
 * Returns the userId string if valid, or null after sending 400.
 */
function _requireValidUserId(req, res) {
  const { userId } = req.body || {}
  if (!userId || !USER_ID_RE.test(userId)) {
    res.status(400).json({ error: 'userId is required and must be a valid 24-char hex string' })
    return null
  }
  // Cross-check with x-user-id header if present
  const headerUserId = req.headers['x-user-id']
  if (headerUserId && headerUserId !== userId) {
    res.status(400).json({ error: 'userId mismatch between header and body' })
    return null
  }
  return userId
}
...
// Internal API handlers for AI agent (Basic Auth, userId from body)

async function internalAddDoc(req, res, next) {
  const projectId = req.params.Project_id
  const { name, parent_folder_id: parentFolderId } = req.body

  if (!isValidObjectId(projectId)) {
    return res.status(400).json({ error: 'Project_id must be a valid 24-char hex ObjectId' })
  }
  if (parentFolderId && !isValidObjectId(parentFolderId)) {
    return res.status(400).json({ error: 'parent_folder_id must be a valid 24-char hex ObjectId' })
  }

  const userId = _requireValidUserId(req, res)
  if (!userId) return
  ...
  if (!canWrite) {
    return res
      .status(403)
      .json({ error: 'user does not have write access to project' })
  }
  if (!_nameIsAcceptableLength(name)) {
    return res.sendStatus(400)
  }
  ...
  try {
    ...
    if (err.message === 'project_has_too_many_files') {
      res.status(400).json({ error: 'project_has_too_many_files' })
    } else {
      next(err)
    }
  }
...
const ALLOWED_ENTITY_TYPES = new Set(['doc', 'file', 'folder'])
...
async function internalDeleteEntity(req, res, next) {
  ...
  if (!isValidObjectId(entityId)) {
    return res.status(400).json({ error: 'entity_id must be a valid 24-char hex ObjectId' })
  }
