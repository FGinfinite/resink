...
describe('ClsiManager', function () {
  ...
  describe('getOutputFileStream', function () {
    beforeEach(async function (ctx) {
      ctx.Settings.apis.clsi.downloadHost = 'http://clsi-nginx.example.com:8080'
      ctx.FetchUtils.fetchStream.resolves('stream')
      ctx.result = await ctx.ClsiManager.promises.getOutputFileStream(
        ctx.project._id,
    ...
    it('should fetch output files from the configured download host', function (ctx) {
      expect(ctx.result).to.equal('stream')
      expect(ctx.FetchUtils.fetchStream).to.have.been.calledWith(
        sinon.match(url => {
          return (
            url.host === 'clsi-nginx.example.com:8080' &&
            url.pathname ===
              `/project/${ctx.project._id}/user/123456123456123456123456/build/abcd-1234/output/output.pdf` &&
            url.searchParams.get('clsiserverid') === 'node-3'
          )
        }),
        sinon.match.object
      )
